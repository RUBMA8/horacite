...
class User {
    ...
    /**
     * Calculer le prochain matricule disponible selon le rôle
     * admin → ADMIN001, ADMIN002, ...
     * responsable → RESP001, RESP002, ...
     */
    static async getNextMatricule(role) {
        const db = await getDatabase();
        const prefix = role === 'admin' ? 'ADMIN' : 'RESP';
        const row = await db.get(
            `SELECT matricule FROM users WHERE matricule LIKE ?
             ORDER BY matricule DESC LIMIT 1`,
            [`${prefix}%`]
        );
        if (!row) return `${prefix}001`;
        const num = parseInt(row.matricule.replace(prefix, ''), 10);
